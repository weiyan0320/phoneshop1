	...
	$(function() {
		...
		$('#myform1').validate({
			...
			submitHandler:function(form){
				//ajax提交登陆信息
				var data = $("#myform1").serialize(); //表单数据
		        $.ajax({
					"async":false,
					"type": "POST",
					...
					"success" : function(data) {//对后台返回的json数据进行处理
						if(data.code=="00"){
							//登陆成功
							layer.msg("登陆成功,跳转到首页！",{icon:1,anim:2,time:2000},function(){
								 window.location.href = "${pageContext.request.contextPath }/index";//跳转到登陆页面 
							});
						}else{
							layer.msg("登陆失败,"+data.message,{icon:5,anim:6,time:3000});
							//登陆失败显示错误信息
