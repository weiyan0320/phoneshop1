...
<body>
    ...
    <div  class="container">
       <div  class="row">
          <div style="padding-top:10px;padding-left:15px;background-color:#f5f5f5;margin-bottom:10px;height:50px;" >
			...
			<p><strong style="font-size:20px;">待付款订单</strong>
				   ...
				   <script type="text/javascript">
				      function deleteThisOrder(oid)
				      {
				    	  layer.confirm("你确定要删除该订单？",function(){
                              //将订单号oid传输到后台删除该订单
			    		      location.href="${pageContext.request.contextPath }/deleteThisOrder?status=1&oid="+oid;
                             ...
                             <tr >
                                  ...
                                  <td colspan="4" width="15%" style="vertical-align:middle;text-align:right;" >
                                          ...
                                          <c:if test="${order.payment_type==2}">
                                            <button type="submit" class="btn btn-info" onclick="topay('${order.oid }')">立即支付</button>&nbsp;&nbsp;
                                            <p>在线支付
                                            <span>
                                                <c:if test="${order.payment_status==0 }">
                                                   (未付款)
	   ...
	   <c:if test="${pageBeanOrder.list!=null }">
	       ...
	       <div style="text-align: center;">
			<ul class="pagination">
			     ...
			     <!-- 首页 -->
			     <li><a href="${pageContext.request.contextPath }/showReadyPayOrder?currentPage=1">首页</a></li>
				 ...
				 <!-- 如果不是第一页 -->
				  <c:if test="${pageBeanOrder.currentPage!=1 }">
				     <!-- 如果当前页不是第一页，选它可以点击-->
				     <li>
				        <a href="${pageContext.request.contextPath }/showReadyPayOrder?currentPage=${pageBeanOrder.currentPage-1}" aria-label="Previous"><span aria-hidden="true">上一页</span></a>
				     </li>
				 </c:if>
				...
				<!-- 根据后台的提供分页数据信息获取进行动态分页 -->
			     <c:forEach begin="1" end="${pageBeanOrder.totalPage }" var="page">
			        ...
			        <c:if test="${page==pageBeanOrder.currentPage }">
			            ...
			            <li class="active"><a href="${pageContext.request.contextPath }/showReadyPayOrder?currentPage=${page }"> ${page } </a></li>
			        </c:if>
			        
			        <!-- 其他不是当前页,不被选中状态 -->
			        <c:if test="${page!=pageBeanOrder.currentPage }">
					
					    <li><a href="${pageContext.request.contextPath }/showReadyPayOrder?currentPage=${page }"> ${page } </a></li>
				...
				<c:if test="${pageBeanOrder.currentPage!=pageBeanOrder.totalPage }">
				    <!-- 如果当前页不是最后一页，让它可以点-->
				    <li>
				      <a href="${pageContext.request.contextPath }/showReadyPayOrder?currentPage=${pageBeanOrder.currentPage+1}" aria-label="Next"> <span aria-hidden="true">下一页</span></a>
				    </li>
				</c:if>
				
				<!-- 尾页 -->
			    <li><a href="${pageContext.request.contextPath }/showReadyPayOrder?currentPage=${pageBeanOrder.totalPage }">尾页</a></li>
    ...
    <script type="text/javascript">
       //立即支付
       function topay(oid){
    	   window.location.href = "${pageContext.request.contextPath }/orderPay?orderNumber="+oid;
       }
    </script>
</body>
