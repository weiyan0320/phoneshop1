...
<body>
    ...
    <div  class="container">
       <div  class="row">
          <div style="padding-top:10px;padding-left:15px;background-color:#f5f5f5;margin-bottom:10px;height:50px;" >
			...
			<p><strong style="font-size:20px;">待收货订单</strong>
		...
		<div class="row">
			<div style="margin: 0 auto; margin-top: 10px;">
				   ...
				   <script type="text/javascript">
				      ...
				      {
				    	  layer.confirm("你确定要删除该订单？",function(){
                              //将订单号oid传输到后台删除该订单
			    		      location.href="${pageContext.request.contextPath }/deleteThisOrder?status=2&oid="+oid;
				   ...
				   <c:forEach items="${pageBeanOrder.list }" var="order">
					  <table class="table table-bordered">
						<tbody>
						    <tr class="active">
								收货人:${order.receiver_name }&nbsp;&nbsp;电话:${order.receiver_phone }&nbsp;&nbsp;收货地址:${order.receiver_address }
							</tr>
							<tr class="active">
								物流订单号:${order.express_no }
							</tr>
							<tr class="active">
							    <th colspan="1"><img src="img/order-time.png">${order.ordertime }&nbsp;&nbsp;手机销售商城<img src="img/order-icons.png"></th>
							...
							<c:forEach items="${order.orderItems }" var="orderItem">
                             ...
                             <tr >
                                  ...
                                  <td colspan="4" width="15%" style="vertical-align:middle;text-align:right;" >
                                      <button type="submit" class="btn btn-info" onclick="readyToReceive('${order.oid }')">确认收货</button>&nbsp;&nbsp;  
                                          ...
                                          <c:if test="${order.payment_type==1}">
                                            <p>货到付款
                                            <c:if test="${order.payment_status==0 }">
                                                   (未付款)
                                             </c:if>
	   ...
	   <c:if test="${pageBeanOrder.list!=null }">
	       ...
	       <div style="text-align: center;">
			<ul class="pagination">
			     ...
			     <!-- 首页 -->
			     <li><a href="${pageContext.request.contextPath }/findReadyToDeliverOrder?currentPage=1">首页</a></li>
				 ...
				 <!-- 如果不是第一页 -->
				  <c:if test="${pageBeanOrder.currentPage!=1 }">
				     <!-- 如果当前页不是第一页，选它可以点击-->
				     <li>
				        <a href="${pageContext.request.contextPath }/findReadyToDeliverOrder?currentPage=${pageBeanOrder.currentPage-1}" aria-label="Previous"><span aria-hidden="true">上一页</span></a>
				     </li>
				 </c:if>
				...
				<!-- 根据后台的提供分页数据信息获取进行动态分页 -->
			     <c:forEach begin="1" end="${pageBeanOrder.totalPage }" var="page">
			        ...
			        <c:if test="${page==pageBeanOrder.currentPage }">
			            ...
			            <li class="active"><a href="${pageContext.request.contextPath }/findReadyToDeliverOrder?currentPage=${page }"> ${page } </a></li>
			        </c:if>
			        
			        <!-- 其他不是当前页,不被选中状态 -->
			        <c:if test="${page!=pageBeanOrder.currentPage }">
					
					    <li><a href="${pageContext.request.contextPath }/findReadyToDeliverOrder?currentPage=${page }"> ${page } </a></li>
				...
				<c:if test="${pageBeanOrder.currentPage!=pageBeanOrder.totalPage }">
				    <!-- 如果当前页不是最后一页，让它可以点-->
				    <li>
				      <a href="${pageContext.request.contextPath }/findReadyToDeliverOrder?currentPage=${pageBeanOrder.currentPage+1}" aria-label="Next"> <span aria-hidden="true">下一页</span></a>
				    </li>
				</c:if>
				
				<!-- 尾页 -->
			    <li><a href="${pageContext.request.contextPath }/findReadyToDeliverOrder?currentPage=${pageBeanOrder.totalPage }">尾页</a></li>
    ...
    <script type="text/javascript">
      //确认收货
      function readyToReceive(oid){
    	  layer.confirm("你确定收货吗？",function(){
    		  $.ajax({
					"async":false,
					"type": "POST",
					"url": "${pageContext.request.contextPath }/readyToReceive",
					"data" : {'oid':oid},
					"success" : function(data) {//对后台返回的json数据进行处理
						if(data.code=="00"){
							//登陆成功
							layer.msg("确认收货成功！",{icon:1,anim:2,time:2000},function(){
								 window.location.href = "${pageContext.request.contextPath }/findReadyToReceiveOrder";//跳转到登陆页面 
							});
						}else{
							layer.msg("确认收货失败!",{icon:5,anim:6,time:3000});
						}
