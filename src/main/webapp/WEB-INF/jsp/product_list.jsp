...
<body>
	...
	<div class="container">
		<div class="row">
		<!-- 如果分页对象里面存储product对象的list集合不为空就显示当前位置 -->
           <c:if test="${!empty pageBean.list}">
			  <div style="border: 1px solid #e4e4e4; margin-bottom: 10px; margin: 0 auto; padding: 10px; margin-bottom: 10px;">
					...
					<c:forEach items="${pageBean.list }" var="product" begin="0" end="0">
                       &gt;<a
							href="${pageContext.request.contextPath }/productListInfo?cid=${product.category.cid} ">${product.category.cname }&nbsp;&nbsp;</a>
					</c:forEach>	
			  </div>
         </c:if>
		</div>
	...
	<ul class="pagination" style="margin-top: 10px;">
	    <!-- 首页 -->
           <li><a href="${pageContext.request.contextPath }/productListInfo?cid=${cid}&currentPage=1">首页</a></li>
		...
		<!-- 如果当前页不是第一页,用户可以点击 -->
		<c:if test="${pageBean.currentPage!=1}">
		<li><a href="${pageContext.request.contextPath }/productListInfo?cid=${cid}&currentPage=${pageBean.currentPage-1}" aria-label="Previous"><span aria-hidden="true">上一页</span></a></li>
		</c:if>
		...
		<c:forEach begin="1" end="${pageBean.totalPage }" var="page"  >
		    <!-- 通过循环遍历每个页面，从1-总页数，后台默认当前页是1，如果用户没有点击循环从1开始，那么1就被选中
		     ...
		     <c:if test="${page==pageBean.currentPage }">
		        <li class="active"><a href="${pageContext.request.contextPath }/productListInfo?cid=${cid}&currentPage=${page}"> ${page } </a></li>
		     </c:if>
		     <!-- 没有点击的其他页肯定和后台存储的当前页不相等，没让它被选中 -->
		     <c:if test="${page!=pageBean.currentPage }">
		        <li><a href="${pageContext.request.contextPath }/productListInfo?cid=${cid}&currentPage=${page}"> ${page } </a></li>
		     </c:if>
		...
		<!-- 当点击下一页时候，如果不是最后一页 -->
		<c:if test="${pageBean.currentPage!=pageBean.totalPage }">
		   <li><a href="${pageContext.request.contextPath }/productListInfo?cid=${cid}&currentPage=${pageBean.currentPage+1}" aria-label="Next"> <span aria-hidden="true">下一页</span></a></li>
		</c:if>
		
           
           <!-- 尾页 -->
           <li><a href="${pageContext.request.contextPath }/productListInfo?cid=${cid}&currentPage=${pageBean.totalPage}">尾页</a></li>
	
	    <!-- 显示一下共有多少页数据 -->
	    <li class="disabled"> <span>共${pageBean.totalPage }页 </span></li>
	    
	     <!-- 显示一下共有多少页数据 -->
	    <li class="disabled"> <span>共${pageBean.totalCount }条 </span></li>
	  </ul>
    </div>
	<!-- 分页结束 -->
  </c:if>
